...
class Agent {
  ...
  initializeDefaultFetchOptions(settings) {
    ...
    const authSettings = (settings && settings.auth) || {};
    const defaultAgentOptions = Object.fromEntries(
      AGENT_OPTIONS.filter((k) => Object.prototype.hasOwnProperty.call(authSettings, k)).map((k) => [k, authSettings[k]])
    );
    const defaultFetchOptions = {};

    if (Object.keys(defaultAgentOptions).length > 0) {
  ...
  initializeLogger(settings) {
    ...
    let methods = Object.keys(logger);

    if (Object.prototype.hasOwnProperty.call(settings, "logger")) {
      methods.forEach((methodName) => {
        if (typeof (settings.logger && settings.logger[methodName]) !== "function") {
          throw new Error(
  ...
  normalizeBatchResponse(batchResponse, requestsResponses, raw) {
    ...
    if (raw) {
      normalizedBatchResponse = Object.assign(batchResponse, {
        batchResponses: requestsResponses,
      });
    } else {
      normalizedBatchResponse = [].concat(...requestsResponses).map((response) =>
  ...
  getResultPath(isList, result) {
    const hasPath = (obj, path) =>
      path.split(".").every((key) => {
        if (obj === null || obj === undefined || !Object.prototype.hasOwnProperty.call(obj, key)) return false;
        obj = obj[key];
        return true;
      });
    return isList && hasPath(result, this._listResultPath)
  ...
  setServiceVersion(version) {
    const isV4 = typeof version === "string" && version.match(/^4\./);
  ...
  fetch(requestUrl, opts = {}) {
    ...
    normalizedOpts = Object.assign({}, this.defaultFetchOptions, opts);
    ...
    return this.readCookies(requestUrl)
      .then((cookies) => {
        ...
        log.logRequest(
          ...
          counter,
          requestUrl,
          Object.fromEntries(Object.entries(normalizedOpts).filter(([key]) => key !== "agent"))
        );
        return fetch(
          requestUrl,
          Object.assign({ redirect: "manual" }, normalizedOpts)
        );
      })
      .then((res) => {
        response = res;
  ...
  nextRequestUrl(requestedUrl, response) {
    ...
    try {
      ...
      requestUrl = new URL(requestedUrl, response && response.url);
    }
  ...
  readCookies(requestUrl) {
    ...
    return this.cookieJar.getCookieString(cookieUrl);
  }
  ...
  appendHeaders(headers, opts) {
    ...
    if (typeof opts === "object" && opts !== null && typeof headers === "object" && headers !== null) {
      normalizedHeaders = Object.fromEntries(
        Object.entries(headers).filter(([, value]) => value != null)
      );
      if (
        typeof opts.headers === "object" && opts.headers !== null &&
  ...
  saveCookies(response) {
    ...
    return Promise.all(
      cookies
        ...
        .map((cookie) => this.cookieJar.setCookie(cookie, response.url))
    );
  }

  setAuthorizationHeaders(authorizationHeaders) {
    if (typeof authorizationHeaders !== "object" || authorizationHeaders === null) {
