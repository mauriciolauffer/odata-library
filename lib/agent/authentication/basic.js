...
function authenticate(settings, agent, endpointUrl) {
  let username = (settings && settings.auth && settings.auth.username) || "";
  let password = (settings && settings.auth && settings.auth.password) || "";
  let authorization =
...
authenticate.isValidResponse = function (response) {
  return (
    response.status === 200 &&
    typeof response.headers.get("content-type") === "string" &&
