    ...
    .then((responseFromLoginForm) => {
      if (authenticate.checkResponseFromLoginPage(responseFromLoginForm)) {
        ...
        return Promise.reject(
          new Error(
            `SAML Identify provider rejected authentification for ${
              settings && settings.auth && settings.auth.username ? settings.auth.username : ""
            }.`
          )
        );
      }
...
authenticate.processDestinationSystemError = function (
  ...
  if (
    (errorHttp && errorHttp.response && errorHttp.response.header && errorHttp.response.header["content-type"] || "").match(
      "application/xml"
    )
  ) {
    parseXML(
      (errorHttp && errorHttp.response && errorHttp.response.res && errorHttp.response.res.text) || "",
      (errorParseXML, parsedXML) => {
        let customizedError = errorHttp;
        let messages = parsedXML && parsedXML.error && parsedXML.error.message;
        if (Array.isArray(messages) && messages.length > 0) {
          customizedError = new Error(
            messages
              .map((message) => message && message._)
              .join("\n")
          );
        }
...
authenticate.submitRedirectToLoginFormAction = function (
  ...
  if (samlRequest) {
    ...
    formParameters = Array.from({ length: samlForm.elements.length }, (_, i) => i)
      .map((index) => mapFormInputs(samlForm, null, index))
...
authenticate.submitLoginFormAction = function (settings, localAgent, response) {
  ...
  if (errorDiv) {
    ...
    submitLoginFormAction = localAgent.fetch(
      ...
      {
        method: "POST",
        body: Array.from({ length: samlForm.elements.length }, (_, i) => i)
          .map((index) => mapFormInputs(samlForm, null, index))
          .filter(filterFormInputs)
          .map((formParameter) => {
            if (formParameter.name.match(/username$/)) {
              formParameter.value = (settings && settings.auth && settings.auth.username) || "";
            }
            if (formParameter.name.match(/password$/)) {
              formParameter.value = (settings && settings.auth && settings.auth.password) || "";
            }
            formParameter.value = formParameter.value || "";
            return formParameter;
          })
