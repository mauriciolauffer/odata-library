...
class Response extends EventEmitter {
  ...
  process(rawResponse) {
    this.body = null;
    Object.entries(this.parseDivideResponse(rawResponse)).forEach(([responseRawKey, responseRawValue]) => {
  ...
  parseDivideResponse(rawResponse) {
    ...
    return {
      rawHTTPResponse: [].concat(blocks[1], "", blocks[2]),
  ...
  getContentType() {
    ...
    if (this.headers && typeof this.headers === "object") {
      const entry = Object.entries(this.headers).find(
        ([name]) => typeof name === "string" && name.toLowerCase() === "content-type"
      );
  ...
  processHeaderInfo(headersInfo) {
    Object.entries(headersInfo).forEach(([headerInfoKey, headerInfoValue]) => {
  ...
  plain(listResultPath, instanceResultPath) {
    ...
    switch (this.request.responseType) {
      case responseType.COUNT:
        result = parsers.count(this.body);
        break;
      case responseType.LIST: {
        const listParts = listResultPath ? listResultPath.split(".") : [];
        result = listParts.reduce((acc, k) => (acc != null ? acc[k] : undefined), this.body);
        break;
      }
      case responseType.ENTITY: {
        const entityParts = instanceResultPath ? instanceResultPath.split(".") : [];
        result = entityParts.reduce((acc, k) => (acc != null ? acc[k] : undefined), this.body);
