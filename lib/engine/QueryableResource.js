...
class QueryableResource extends Resource {
  ...
  isPlainRequest(body) {
    return (
      ...
      Object.prototype.hasOwnProperty.call(this.defaultRequest._headers, "slug")
    );
  }
  ...
  processUpdateCall(methodName, ...args) {
    ...
    if (args.length === 0 || args.length > 2) {
      ...
      entity = Object.assign({}, args[0], this.defaultRequest._keyValue);
      keyProperties = this.keyProperties(entity);
      keyPredicate = this.keyPredicate(keyProperties);
      propertiesToChange = Object.fromEntries(
        Object.entries(entity).filter(([key]) => !Object.prototype.hasOwnProperty.call(keyProperties, key))
      );
    } else {
  ...
  keyPredicate(entityKey) {
    return Object.entries(entityKey)
  ...
  processProperties(entityTypeProperties, entityTypeModelProperties) {
    return entityTypeModelProperties
      .filter((entityTypeProperty) =>
        Object.prototype.hasOwnProperty.call(entityTypeProperties, entityTypeProperty.name)
      )
  ...
  processNavigationProperties(entityTypeProperties, entityTypeModel) {
    let navigationProperties = (entityTypeModel && entityTypeModel.navigationProperties) || [];
    if (entityTypeModel === null || typeof entityTypeModel !== "object") {
      throw new Error(
    ...
    return navigationProperties
      .filter((navigationProperty) =>
        Object.prototype.hasOwnProperty.call(entityTypeProperties, navigationProperty.name)
      )
      ...
      .reduce(
        (acc, processedNavigationProperties) =>
          Object.assign(acc, processedNavigationProperties),
        {}
      );
  }
  ...
  processNavigationPropertyItems(navigationProperty, entityTypeProperties) {
    const entityTypeModelEnd = navigationProperty && navigationProperty.type && navigationProperty.type.elementType;
    const navigationPropertyItems =
      entityTypeProperties[navigationProperty.name];
    let navigationPropertyItemsProcessed;

    if (entityTypeModelEnd === null || typeof entityTypeModelEnd !== "object") {
      throw new Error(
        `End EntityType for navigation property ${navigationProperty.name} from EntityType ${this.entityTypeModel.name} does not exists.`
      );
    }

    if (navigationProperty.isCollection && Array.isArray(navigationPropertyItems)) {
      navigationPropertyItemsProcessed = navigationPropertyItems.map(
        (navigationPropertyItemsToProcess) => {
          return Object.assign(
            this.processProperties(
              navigationPropertyItemsToProcess,
              (entityTypeModelEnd && entityTypeModelEnd.properties) || []
            ),
      ...
      navigationPropertyItemsProcessed = Object.assign(
        this.processProperties(
    ...
    if (request._isRaw || request._isPlain) {
      ...
      promise = response.json().then((json) => {
        const getPath = (obj, path) =>
          path ? path.split(".").reduce((o, k) => (o != null ? o[k] : undefined), obj) : undefined;
        resultPath = this.agent.getResultPath(request._isList, json);
        const resultValue = getPath(json, resultPath);
        return this._unwrapNestedProperties(
          resultPath ? (resultValue !== undefined ? resultValue : response.ok) : json
        );
      });
    }
  ...
  _unwrapNestedProperties(response) {
    ...
    let skipResultsProperty = (value) =>
      Object.prototype.hasOwnProperty.call(value, "results")
        ...
        : value;
    let mapObjectProperties = (item) =>
      Object.fromEntries(Object.entries(item).map(([k, v]) => [k, skipResultsProperty(v)]));
    let unwrapObject = (item) =>
      item && typeof item === "object" ? mapObjectProperties(item) : item;
    splitResponse = (fn, item) =>
      Array.isArray(item) ? item.map(fn.bind(null, fn)) : unwrapObject(item);
