...
function applyOptionalArgument(request, args) {
  ...
  if (args.length === 1 && typeof args[0] === "number") {
    request.top(args[0]);
  } else if (args.length === 1 && args[0] !== null && typeof args[0] === "object" && Object.getPrototypeOf(args[0]) === Object.prototype) {
    request.key(args[0]);
  } else {
    ok = args.length === 0;
  }
...
class RequestDefinition {
   ...
   */
  key(entityKey) {
    if (entityKey !== null && typeof entityKey === "object" && Object.getPrototypeOf(entityKey) === Object.prototype) {
  ...
  parameters(parameters) {
    if (parameters !== null && typeof parameters === "object" && Object.getPrototypeOf(parameters) === Object.prototype) {
      Object.entries(parameters).forEach(([key, value]) => this.parameter(key, value));
    }
  ...
  select() {
    ...
    if (arguments.length > 0) {
      propertyNames = Array.from(arguments).reduce(
        (acc, property) => acc.concat(property),
        []
      );
  ...
  expand() {
    let paths;
    if (arguments.length > 0) {
      paths = Array.from(arguments).reduce(
        (acc, property) => acc.concat(property),
        []
      );
  ...
  calculatePath() {
    ...
    if (foundResponseType && Object.prototype.hasOwnProperty.call(requestPath.calculate, foundResponseType)) {
  ...
  get _isEntity() {
    ...
    let isNavPropSingle =
      typeof this._resource.isMultiple === "function" && !this._resource.isMultiple();
    return (
      Object.prototype.hasOwnProperty.call(this, "_keyValue") || Object.prototype.hasOwnProperty.call(this, "_payload") || isNavPropSingle
    );
  }
