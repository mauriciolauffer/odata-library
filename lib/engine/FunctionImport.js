...
class FunctionImport extends Resource {
      ...
      if (typeof contentType === "string" && contentType.match(/application\/json/)) {
        promise = rawResponse.json().then((json) => {
          ...
          if (Array.isArray(listValue)) {
            result = listValue;
          } else if (this.agent._instanceResultPath !== "" && getPath(json, this.agent._instanceResultPath) !== undefined) {
   ...
   */
  queryFromParameters() {
    let queryParameters = Object.entries(this.defaultRequest._query).map(([key, value]) => {
      return `${key}=${value}`;
    });

    let parameters = Object.entries(this.defaultRequest._parameters).map(([key, value]) => {
      return `${key}=${value}`;
    });
    return queryParameters.concat(parameters).join("&");
  }
}
