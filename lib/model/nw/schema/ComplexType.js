...
class ComplexType extends aggregate(AnnotationTarget, SharedComplexType) {
  ...
  constructor(rawMetadata, model) {
    ...
    let properties = (this.raw && this.raw.Property || []).map(
      (p) => new Property(p)
    );
  ...
  getProperty(name, strict = true) {
    ...
    if (propertyNameParts.length === 1) {
      ...
      const matchedNavProps = this.navigationProperties
        ...
        .map((navigationProperty) => {
          ...
          return (associationEnd && associationEnd.ends && associationEnd.ends[1] && associationEnd.ends[1].type && associationEnd.ends[1].type.properties) || [];
        });
      properties = matchedNavProps[0] || [];
    }
  ...
  getLegacyApiObject() {
    return Object.assign(super.getLegacyApiObject(), {
      Properties: Object.fromEntries(
        this.properties.map((p) => [p.getLegacyApiObject().Name, p.getLegacyApiObject()])
      ),
    });
  }
}
