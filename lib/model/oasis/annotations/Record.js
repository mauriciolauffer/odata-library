...
class Record {
  ...
  constructor(rawMetadata, expressionBuilder) {
    ...
    let annotations = (this.raw && this.raw.Annotation || []).map(
    ...
    let propertyValues = (this.raw && this.raw.PropertyValue || []).map(
    ...
    let value = Object.fromEntries(propertyValues.map((pv) => [pv.property, pv]));
