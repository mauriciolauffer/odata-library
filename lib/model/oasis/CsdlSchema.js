...
class CsdlSchema {
  ...
  resolveModelPath(path) {
    ...
    let child = childCollections
      .map((collection) =>
        this[collection.name].find(
          (item) =>
            (item.name === target.element ||
            (typeof item.matchModelPath === "function" && item.matchModelPath(target)))
        )
      )
      .filter(Boolean);
  ...
  applyAnnotations(annotationsData) {
    if (annotationsData && this.raw.EntityContainer) {
      ...
      let annotationGroups = validItems.reduce((acc, annotation) => {
        const key = annotation.$.Target;
        if (!acc[key]) acc[key] = [];
        acc[key].push(annotation);
        return acc;
      }, {});
      Object.entries(annotationGroups).forEach(([path, annotations]) =>
        this._applyAnnotationsToPath(annotations, path)
      );
    }
  }
  ...
  static initChildProperties(schema, metaModel) {
    childCollections.forEach((collection) => {
      let child = (schema.raw[collection.sourceElement] || []).map(
        (typeRawMetadata) => {
