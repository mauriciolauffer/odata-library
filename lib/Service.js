      ...
      value: new Promise((resolve, reject) => {
        this.agent
          ...
          .then((responses) => {
            ...
            functionImports =
              typeof metadata.model.version === "string" &&
              parseInt(metadata.model.version.substring(0, 1), 10) >= 4
...
${metaFunction.name} will not be accessible.`
  ...
  createChangeSet(batch) {
    let batchNormalized =
      batch || (this.agent.batchManager && this.agent.batchManager.defaultBatch);
    ...
    [
      ...
      [
        () => {
          return (
            arguments.length === 0 &&
            !(this.agent && this.agent.batchManager && this.agent.batchManager.defaultBatch)
          );
        },
      ...
      [
        () => {
          return (
            arguments.length === 0 &&
            !(this.agent && this.agent.batchManager && this.agent.batchManager.defaultBatch && this.agent.batchManager.defaultBatch.defaultChangeSet)
          );
        },
